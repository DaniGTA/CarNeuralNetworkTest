    ...
    public class Network
    {
        ...
        public Network(AIController aIController,params Network[] networks)
        {
            ...
            for (int i = 0; i < parameters.Length - 1; i++)
            {
                ...
                for (int j = 0; j < parameters[i]; j++)
                {
                    ...
                    for (int k = 0; k < parameters[i + 1]; k++)
                    {
                        ...
                        weights[i][j][k] = networks[Random.Range(0,networks.Length)].weights[i][j][k];
                    }
                }
            }
            ...
            for (int i = 0; i < aIController.mutationRateStatic; i++)
            {
                ...
                {
                    weights[mutationLayer][mutationLeft][mutationRight] = weights[mutationLayer][mutationLeft][mutationRight] / 1.00001f;
                }
                else
                {
                    weights[mutationLayer][mutationLeft][mutationRight] = weights[mutationLayer][mutationLeft][mutationRight] * 1.00001f;
